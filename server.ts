...
async function generateContentWithFallback(
  ...
  for (const model of modelsToTry) {
    try {
      ...
      // If temporary high demand, immediately try the next fallback model in the list
      if (isTemporary) {
        await new Promise((resolve) => setTimeout(resolve, 200));
        continue;
      }
      // If another error, still try the next model once before giving up
      continue;
    }
  }
...
app.post("/api/interactions", async (req, res) => {
  try {
    ...
    const defaultSystem = language === "en" 
      ...
      : "أجب عن السؤال أو الطلب مباشرة وبشكل دقيق ومحدد دون مقدمات ترحيبية أو تعريف بالنفس ودون قوائم اقتراحات غير مطلوبة في الختام.";
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const systemInstruction = language === "en"
      ? "You are a concise, direct, and helpful assistant. Provide ONLY the direct, accurate, and relevant answer to the user's prompt or question. Do NOT include promotional greetings, self-introductions (like 'I am EDITOR' or 'as your assistant'), conversational filler, or unrequested closing lists of suggestions and offers. Deliver the exact factual/technical answer directly."
      : "قدّم الإجابة المباشرة والدقيقة والصريحة على سؤال المستخدم فقط دون أي مقدمات ترويجية أو تعريف بالنفس (مثل 'بصفتي مساعدك' أو 'أنا هنا لمساعدتك') ودون عبارات ترحيبية زائدة، وبدون قوائم ختامية غير مطلوبة للاقتراحات أو عروض المتابعة. اجعل الرد مركّزاً بنسبة 100% على صلب الإجابة والمعلومات المطلوبة بدقة.";
    ...
    const fallbackText = language === "en"
      ? `Hello! I am EDITOR AI Assistant powered by Google Interactions API. I have received your message: "${lastUserMessage.slice(0, 100)}" and I am ready to help with code design, APK creation, translation, and file analysis.`
      : `أهلاً بك! أنا مساعد EDITOR الذكي المدعوم ببروتوكول Google Interactions API. تلقيت طلبك: "${lastUserMessage.slice(0, 100)}" وجاهز لمساعدتك في تصميم الأكواد، وبناء برامج APK، والترجمة، ومعالجة الملفات.`;
    ...
    // Resilient fallback instead of hard error
...
app.post("/api/translate", async (req, res) => {
  try {
    ...
    const { text = "", targetLang = "en" } = req.body;
    res.json({
      translatedText: targetLang === "en" ? `[Translated]: ${text}` : `[ترجمة]: ${text}`,
      targetLang,
    });
  }
});
...
app.post("/api/generate-code", async (req, res) => {
  try {
    ...
    if (ai && description) {
      const prompt = `Write high quality, production-ready, clean and fully working ${language} code for: "${description}". Framework/library: ${framework || "standard"}. Include comments and instructions.`;
      const response = await generateContentWithFallback(ai, {
        preferredModel: "gemini-3.7-flash",
    ...
    res.json({
      code: `// ${language.toUpperCase()} Code Example for: ${description}\nconsole.log("EDITOR Code Studio - ${language}");`,
      language
    });
  } catch (e: any) {
    const { language = "javascript", description = "" } = req.body;
    res.json({
      code: `// ${language.toUpperCase()} Code for: ${description}\n// Generated by EDITOR Code Studio\n\nfunction main() {\n  console.log("Code generated successfully for: ${description}");\n}\nmain();`,
      language
    });
  }
});
  ...
  try {
    ...
    if (ai && (lastResponse || lastPrompt)) {
      const promptText = language === "en"
        ? `Based on the following discussion topic and answer, provide 3 highly professional, realistic, and actionable solutions or technical follow-ups tailored specifically to the subject discussed on screen.
...
Return strictly JSON format: {"suggestions": ["Professional Solution 1", "Professional Solution 2", "Professional Solution 3"]}`
        : `بناءً على موضوع النقاش والإجابة المعروضة على الصفحة، اقترح 3 حلول احترافية حقيقية وعملية ومباشرة ترتبط بدقة بالموضوع المطروح وتطويره.
